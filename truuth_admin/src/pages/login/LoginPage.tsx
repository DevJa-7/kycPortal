...
import { clearGeneralCookies } from 'shared_components/src/service/common.service';
import { NOTIFICATION_STATES, APP_URL_PREFIX } from 'shared_components/src/common/constants';
...
const LoginPage = () => {
	...
	const setUserInfo = (res: any, isReset = false) => {
		...
		if (checkUser(authInfo.roles)) {
			...
			gotoStartPage(history, authInfo.roles);
		} else {
			if (showReset) {
				openReset();
			} else {
	...
	return (
		<RootContainer>
			...
			<Grid container item sm={5} className={classes.itemNoFit}>
				<Login errors={errors} handleSignIn={handleSignIn} openForgotPassword={openForgotPassword} />
				<CompleteNewPassword
